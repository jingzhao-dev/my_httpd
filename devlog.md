...
## 2026-06-06 阶段二-B完成：哈希表集成到 serve_file.c

### 完成内容
- 在 `serve_file.c` 中引入 `hashmap.h`，用懒初始化方式创建全局哈希表 `mime_map`
- 修改 `get_mime_type` 函数：将结构体数组线性查找（O(n)）替换为 `hashmap_get` 查找（O(1)）
- 修改 `Makefile`，添加 `HASHMAP_OBJ` 变量并链接 `hashmap.o`
- 编译完整服务器，浏览器和 curl 测试通过：HTML、CSS、文本、图片等文件的 Content-Type 均正确
- 通过 valgrind 内存检查，确认无内存泄漏
- 删除旧代码：`mime_map_t` 结构体定义、`mime_map` 数组、旧的 `for` 循环遍历逻辑
- 打 tag v1.3 并推送到 GitHub

### 亲手解决的、印象深刻的问题

**1. 文件作用域不能写函数调用——C 语言的执行模型**

最初想在 `#include` 下方直接写 `hashmap_put(mime_map, ".html", "text/html");` 来填充哈希表，编译器报错。原因：C 语言规定所有可执行语句必须出现在函数体内部。文件作用域只能写声明、定义、初始化——不能写函数调用。

`main` 是程序的唯一入口。在 `main` 被调用之前，不存在运行环境来执行文件作用域的函数调用。全局变量的初始化是一个例外——编译器在编译期为它们分配空间并填入初始值，但 `hashmap_put` 是运行时函数调用，必须在某个函数体的上下文中执行。

**2. 为什么需要全局变量——数据结构的生命周期**

哈希表存储的是 12 个永远不变的 MIME 映射关系。如果每次调用 `get_mime_type` 都重新创建哈希表、插入 12 个条目、用一次就销毁——这意味着每个 HTTP 请求要做 12 次内存分配和释放，只为查一次扩展名。

这显然不合理。需要的是一个持久化的数据结构——创建一次，整个服务器生命周期内复用。全局变量（`static` 文件作用域）正是表达"持久化"的 C 语言原生方式。这不是"全局变量有害"教条的反例，而是持久化数据的正当用途。

**3. 初始化与赋值的本质区别**

| 概念 | 发生时机 | 语法形式 |
|------|---------|---------|
| 初始化 | 变量诞生时（创建那一刻） | `int x = 5;` |
| 赋值 | 变量已存在后的某个时刻 | `x = 5;` |

全局变量的初始化发生在程序加载时，初始值必须在编译期确定。`static HashMap *mime_map = NULL;` 是初始化——程序启动时 `mime_map` 就是 `NULL`。而 `mime_map = hashmap_create_with_capacity(31);` 是赋值——发生在 `get_mime_type` 首次被调用时。

`static` 局部变量的初始化只执行一次（第一次执行到这一行时），后续调用跳过初始化。这个特性正是懒初始化依赖的机制。不理解这两者的区别，就无法理解为什么 `static int x = 0; x = 5;` 中第一行只执行一次，第二行每次执行。

**4. 懒初始化的逻辑反了——调试时的"大脑翻转"**

在 `get_mime_type` 中，`hashmap_get` 返回 `NULL` 表示没找到，应该返回默认类型；返回非 `NULL` 表示找到了，应该返回查到的值。但最初把两个分支写反了——找到了返回默认类型，找不到返回 `NULL`。

这是一个典型的"逻辑反转"错误。排查时的关键线索：浏览器请求 `index.html` 时 Content-Type 变成了 `application/octet-stream`（默认兜底类型），而不是 `text/html`。说明找到了的值没有被返回，反而走了默认分支。条件判断写反是调试中最容易定位也最容易犯的错误——编译器帮不了你，只有测试数据能告诉你。

**5. 变量命名拼写错误——`mine_map` vs `mime_map`**

把 `mime_map` 写成了 `mine_map`。MIME 是标准术语（Multipurpose Internet Mail Extensions），拼写错误虽然不影响编译，但会在后续代码阅读和搜索时制造麻烦。变量名拼写是最容易被忽视的 bug 源——它们不会导致编译错误，但会在 grep 搜索、代码补全、团队协作时持续消耗注意力。

**6. 端口占用的 TIME_WAIT 机制**

连续启动服务器时遇到 `bind: Address already in use`。原因：TCP 协议规定主动关闭连接的一方必须等待 2MSL（约 60-120 秒）才能完全释放端口，防止网络中残留的旧数据包被误传给新启动的服务器。

解决方案：在 `socket.c` 的 `bind` 之前使用 `setsockopt` 设置 `SO_REUSEADDR`，允许端口在 TIME_WAIT 状态下被重新绑定。这是所有生产级服务器程序的标准做法。

### 关键认知突破

**1. 懒初始化模式——在数据使用点管理生命周期**

不单独创建 `mime_map_init()` 函数供 `main` 调用，而是在 `get_mime_type` 内部检查并初始化。好处是改造成本最小——`get_mime_type` 的调用者（`serve_static_file_buf`）不需要知道哈希表的存在。这是一种降低耦合的技术：数据的创建和使用在同一个函数内，外部不可见。

代价是首次调用时有一个初始化开销（12 次内存分配），后续调用只需一次哈希查找。对于单线程服务器，这个开销完全可以接受。

**2. 哈希表补齐了项目的通用数据结构拼图**

和 `DynBuf` 一样，`HashMap` 是独立于 HTTP 服务器的通用模块，放在 `~/clib` 中。两者一起构成了项目的自建基础库：

| 数据结构 | 文件 | 用途 |
|----------|------|------|
| DynBuf | 动态字节缓冲区 | 构造 HTTP 响应 |
| HashMap | 字符串键值对哈希表 | MIME 类型查找 |

这些数据结构不依赖任何 HTTP 逻辑，可以在任何 C 项目中复用。

**3. 从 O(n) 到 O(1) 的实际意义**

12 个 MIME 类型的线性查找在性能上完全可以忽略不计。但这次改造的意义不在这里——它验证了"用自定义数据结构替换简陋实现"这个模式是可行的。DynBuf 替换固定数组，HashMap 替换结构体数组线性查找——两次改造遵循了相同的流程：独立开发 → 独立测试 → 集成 → 删除旧代码。

这个模式将在阶段二-C 中继续使用——用链表替换请求头解析中的简陋实现。

### 阶段二-B 完成总结

| 改造项 | 旧实现 | 新实现 | 收益 |
|--------|--------|--------|------|
| MIME 存储 | `mime_map_t` 结构体数组 | `HashMap *` 全局哈希表 | 封装性更好，不再暴露内部结构 |
| MIME 查找 | `for` 循环遍历数组 | `hashmap_get` O(1) 查找 | 查找效率提升，代码更简洁 |
| MIME 初始化 | 编译期静态数组初始化 | 运行时懒初始化 | 灵活，可动态增删 |

### 当前项目状态

- Git tag: v1.3（哈希表集成完成）
- 服务器功能完整：静态文件服务、CGI 支持、路径安全防护、结构化日志
- 核心数据结构：DynBuf（动态字节缓冲区）、HashMap（通用哈希表）
- 所有模块均已切换到新接口，旧代码已清理

### 下一步计划（阶段二-C）
- 在 `~/clib` 中实现通用链表（字符串键值对链表）
- 在 `parser.c` 中增加 `parse_headers` 函数，解析 HTTP 请求头（Host、User-Agent、Content-Length 等）
- 为 CGI 处理 POST 请求打下基础